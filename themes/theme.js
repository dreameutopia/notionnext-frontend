...
// 限制允许的主题（新增）
// ============================================
const ALLOWED_THEMES = BLOG.ALLOWED_THEMES || ['heo', 'gitbook', 'typography']

/**
 * 验证主题是否允许使用
 */
function validateTheme(theme) {
  if (!theme) return BLOG.THEME || 'heo'
  return ALLOWED_THEMES.includes(theme) ? theme : (BLOG.THEME || 'heo')
}
...
export const getThemeConfig = async themeQuery => {
  // 验证主题
  const validatedTheme = validateTheme(themeQuery)
  
  // 如果 themeQuery 存在且不等于默认主题，处理多主题情况
  if (validatedTheme && validatedTheme !== BLOG.THEME) {
...
export const getBaseLayoutByTheme = theme => {
  ...
  const isDefaultTheme = !validatedTheme || validatedTheme === BLOG.THEME
  
  if (!isDefaultTheme) {
    return dynamic(
      () => import(`@/themes/${validatedTheme}`).then(m => m['LayoutBase']),
      { ssr: true }
    )
  }
